...
export default async function handler(req: VercelRequest, res: VercelResponse) {
  ...
  try {
    ...
    const { error } = await srv.from('profiles').update({ is_banned: false }).eq('user_id', user_id)
    if (error) return res.status(500).json({ error: `profiles update failed: ${error.message}` })
