...
export default async function handler(req: VercelRequest, res: VercelResponse) {
  ...
  try {
    ...
    const { data, error } = await srv.auth.admin.inviteUserByEmail(email)
    if (error) return res.status(500).json({ error: `admin.inviteUserByEmail failed: ${error.message}` })
    return res.json({ invited: true, user: data.user })
